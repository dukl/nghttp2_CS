...
static void run(const char *service, const char *key_file,
  ...
  ssl_ctx = create_ssl_ctx(key_file, cert_file);//key and cert files 和发送的内容 无关 ,所以 ssl_ctx与发送内容无关
...
void Server()
{
  ...
  if(pthread_create(&t0,NULL,thread_b,NULL)==-1)
  {
      exit(1);
  }
  /*run(argv[1], argv[2], argv[3]);//这一步在输出*/
  void* result;
